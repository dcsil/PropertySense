...
import 'package:object_detect_test/ui/viewmodels/offers_homeowner_viewmodel.dart';
import 'package:provider/provider.dart';

class OffersHomeownerScreen extends StatelessWidget {
  const OffersHomeownerScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => OffersHomeownerViewModel(
        context.read<ListingRepository>(),
        context.read<UserRepository>(),
      ),
      child: const OffersHomeownerScreenContent(),
    );
  }
}

class OffersHomeownerScreenContent extends StatelessWidget {
  const OffersHomeownerScreenContent({super.key});

  @override
  Widget build(BuildContext context) {
    final viewModel = context.watch<OffersHomeownerViewModel>();
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => context.push('/profile-homeowner'),
          ),
        ],
      ),
      body: Column(
            ...
            child: Row(
              children: [
                ...
                FilterChip(
                  ...
                  selected: viewModel.selectedFilter == OfferStatus.rejected ||
  ...
  Widget _buildOffersBody(BuildContext context, OffersHomeownerViewModel viewModel) {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return _OfferCard(
            offer: offer,
            listingTitle: viewModel.listingIdMap[offer.listingId]?.title ?? 'Unknown',
            onTap: () {
              if (offer.status == OfferStatus.pending) {
                _showAcceptOfferDialog(context, viewModel, offer);
              } else if (offer.status == OfferStatus.accepted) {
                _showCompleteAppointmentDialog(context, viewModel, offer);
              }
            },
          );
        },
  ...
  void _showAcceptOfferDialog(
    ...
    OffersHomeownerViewModel viewModel,
    Offer offer,
  ) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(
                'Listing: ${viewModel.listingIdMap[offer.listingId]?.title ?? 'Unknown'}',
              ...
              _DetailRow(
                ...
                label: 'Offer Price',
                value: '\$${offer.offerPrice.toStringAsFixed(0)}',
              ),
              const SizedBox(height: 8),
              if (offer.offerDate != null)
                _DetailRow(
                  ...
                  label: 'Proposed Date',
                  value: DateFormat('MMMM d, yyyy').format(offer.offerDate!.toDate()),
                ),
              const SizedBox(height: 8),
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              await viewModel.acceptOffer(offer.id, offer.listingId);
            },
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'This will reject all other pending offers and appointments for this listing.',
              style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                    color: Theme.of(context).colorScheme.onSurfaceVariant,
                  ),
            ),
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              Navigator.pop(context);
              await viewModel.completeAppointment(offer.id, offer.listingId);
            },
                    ...
                    if (offer.offerDate != null) ...[
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            'Date: ${DateFormat('MMM d, yyyy').format(offer.offerDate!.toDate())}',
                            style:
