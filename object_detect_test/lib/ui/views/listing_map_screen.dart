...
class ListingMapScreenContent extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final viewModel = context.watch<ListingMapViewModel>();
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          _buildMapBody(context, viewModel),
          
          // New listing popup
          if (viewModel.newListing != null)
            Positioned(
              bottom: 16,
              right: 16,
              child: _NewListingPopup(
                listing: viewModel.newListing!,
                onDismiss: viewModel.dismissNewListing,
                onOfferCreated: (offer) async {
                  await viewModel.createOffer(offer);
                  viewModel.dismissNewListing();
                },
                contractorId: viewModel.currentUserId,
              ),
            ),
        ],
      ),
  ...
  Widget _buildMapBody(BuildContext context, ListingMapViewModel viewModel) {
    ...
    return FlutterMap(
      ...
      children: [
        ...
        // Listing markers
        MarkerLayer(
          markers: viewModel.listings.map((listing) {
            return Marker(
              point: LatLng(
...
class _NewListingPopup extends StatelessWidget {
  final Listing listing;
